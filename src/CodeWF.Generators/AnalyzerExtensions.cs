using CodeWF.Generators.Analyzers;
...
public static class AnalyzerExtensions
{
    public static List<IdentifierNameSyntax> GetUiContextReferences(this SyntaxNode node, SemanticModel semanticModel)
    {
        var directReferences =
            node.DescendantNodes()
                ...
                .Where(x => semanticModel.GetTypeInfo(x).Type?.ToDisplayString() ==
                            UiContextAnalyzer
                                .UiContextType) // slower, but safer. Only runs if previous verification passed.
                .ToList();
    ...
    {
        return node.IsSubTypeOf(model, "CodeWF.ViewModels.Navigation.RoutableViewModel");
    }

    public static (string? TypeName, IEnumerable<string> Namespaces) GetDialogResultType(this SyntaxNode node,
        SemanticModel model)
    {
        if (node is not ClassDeclarationSyntax cls)
        {
